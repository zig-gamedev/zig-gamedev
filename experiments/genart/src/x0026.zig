...
pub const name = "generative art experiment: x0026";
...
pub fn draw() void {
    ...
    for (0..50_000) |_| {
        ...
        const tn1 = @floatFromInt(f64, iter) * v;

        const vel = (xn1 - xn) * (xn1 - xn) + (yn1 - yn) * (yn1 - yn) + random.float(f64) * 0.5;
        if (vel < 0.5) {
            gl.color3f(c, c, c);
        } else if (vel < 2.0) {
            gl.color3f(c, 0.0, 0.0);
        } else {
            gl.color3f(0.0, c * 0.5, c * 0.5);
        }
