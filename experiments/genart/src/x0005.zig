...
pub fn draw() void {
    ...
    for (0..4) |i| {
        gl.pushMatrix();
        gl.rotatef(rot + @floatFromInt(f32, i) * 45.0, 0.0, 0.0, 1.0);
        gl.begin(gl.LINES);
        gl.vertex2f(0.0, -1.0);
        gl.vertex2f(0.0, 1.0);
        gl.end();
    ...
    gl.begin(gl.POINTS);
    for (0..20) |i| {
        const fract = @floatFromInt(f32, i) / 20.0;
    ...
    gl.begin(gl.POINTS);
    for (0..30) |i| {
        const fract = @floatFromInt(f32, i) / 30.0;
    ...
    gl.begin(gl.POINTS);
    for (0..40) |i| {
        const fract = @floatFromInt(f32, i) / 40.0;
