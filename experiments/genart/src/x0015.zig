...
pub const name = "generative art experiment: x0015";
    ...
    if (y <= bounds and pass == 1) {
        ...
        while (row < 4) : (row += 1) {
            ...
            while (x <= bounds) : (x += step) {
                var v = Vec2{ x, y };
                const xoff = random.floatNorm(f32) * 0.0025;
                const yoff = random.floatNorm(f32) * 0.0025;
                const v0 = pdj(v, 1.2);
                const v1 = sinusoidal(.{ v0[0] + v[0], v0[1] + v[1] }, 2.4);
                v = sinusoidal(.{ v0[0] + v1[0], v0[1] + v1[1] }, 2.4);
...
fn julia(v: Vec2, scale: f32, rand01: f32) Vec2 {
    ...
    const theta = 0.5 * math.atan2(f32, v[0], v[1]) +
        math.pi * @floatFromInt(f32, @intFromFloat(i32, 2.0 * rand01));
