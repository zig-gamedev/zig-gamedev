...
pub fn package(
    ...
    const zglfw_c_cpp: ?*std.Build.Step.Compile = if (target.result.os.tag == .emscripten) null else buildLib: {
        const lib = if (args.options.shared) blk: {
            ...
            if (target.result.os.tag == .windows) {
                shared_lib.defineCMacro("_GLFW_BUILD_DLL", null);
            }
        ...
        lib.addIncludePath(.{ .path = thisDir() ++ "/libs/glfw/include" });
        lib.linkLibC();
...
pub fn build(b: *std.Build) void {
    ...
    if (pkg.zglfw_c_cpp) |zglfw_c_cpp| {
        b.installArtifact(zglfw_c_cpp);
    }
}
