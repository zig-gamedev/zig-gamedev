...
pub const Package = struct {
    ...
    pub fn link(zwin32_pkg: Package, exe: *std.Build.CompileStep, libs: Libs) void {
        if (libs.d3d12) exe.step.dependOn(zwin32_pkg.install_d3d12);
        if (libs.xaudio2) exe.step.dependOn(zwin32_pkg.install_xaudio2);
        if (libs.directml) exe.step.dependOn(zwin32_pkg.install_directml);
    }
};
