...
fn init(allocator: std.mem.Allocator) !DemoState {
    ...
    const mesh_textures = [_]ResourceView{
        .{
            .resource = gctx.createAndUploadTex2dFromFile(
    ...
    for (mesh_textures) |texture| {
        ...
        gctx.device.CreateShaderResourceView(gctx.lookupResource(texture.resource).?, null, texture.view);
...
fn draw(demo: *DemoState) void {
    ...
    // Draw SciFiHelmet.
    {
        ...
        const mem = gctx.allocateUploadMemory(PsoMeshPbr_Const, 1);
        mem.cpu_slice[0] = .{
        ...
        gctx.cmdlist.SetGraphicsRootDescriptorTable(1, blk: {
            const table = gctx.copyDescriptorsToGpuHeap(1, demo.mesh_textures[0].view);
            _ = gctx.copyDescriptorsToGpuHeap(1, demo.mesh_textures[1].view);
            _ = gctx.copyDescriptorsToGpuHeap(1, demo.mesh_textures[2].view);
            _ = gctx.copyDescriptorsToGpuHeap(1, demo.mesh_textures[3].view);
            _ = gctx.copyDescriptorsToGpuHeap(1, demo.irradiance_texture.view);
