...
const window_title = "zig-gamedev: minimal zgpu zd3d12";
...
pub fn main() !void {
    ...
    { // init imgui backend
        var font_cbv_srv_heap: ?*d3d12.IDescriptorHeap = null;
        hrPanicOnFail(
            gctx.device.CreateDescriptorHeap(
                &.{
                    .Type = .CBV_SRV_UAV,
                    .NumDescriptors = 1,
                    .Flags = .{},
                ...
                @as(*?*anyopaque, @ptrCast(&font_cbv_srv_heap)),
            ),
        );

        zgui.backend.init(.{
            .hwnd = window,
            .d3d12_device = gctx.device,
            .num_frames_in_flight = 1,
            .rtv_format = @intFromEnum(dxgi.FORMAT.R8G8B8A8_UNORM),
            .cbv_srv_heap = @ptrCast(font_cbv_srv_heap),
            .font_srv_cpu_desc_handle = @intCast(
                font_cbv_srv_heap.?.GetCPUDescriptorHandleForHeapStart().ptr,
            ),
            .font_srv_gpu_desc_handle = @intCast(
                font_cbv_srv_heap.?.GetGPUDescriptorHandleForHeapStart().ptr,
            ),
        });
    }
