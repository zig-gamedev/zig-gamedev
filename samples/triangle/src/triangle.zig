...
pub const GraphicsContext = struct {
    ...
    cmdlist: *w.ID3D12GraphicsCommandList3,
    ...
    pub fn init(window: w.HWND) !GraphicsContext {
        ...
        const device = blk: {
            var maybe_device: ?*w.ID3D12Device2 = null;
            try vhr(w.D3D12CreateDevice(null, ._11_1, &w.IID_ID3D12Device2, @ptrCast(*?*c_void, &maybe_device)));
        ...
        const cmdlist = blk: {
            var maybe_cmdlist: ?*w.ID3D12GraphicsCommandList3 = null;
            try vhr(device.CreateCommandList(
                0,
                .DIRECT,
                cmdallocs[0],
                null,
                &w.IID_ID3D12GraphicsCommandList3,
